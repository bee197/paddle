    ...
    def step(self, action):
        ...
        if video_in:
            ...
            if angle > 0.24:
                # 角度靠近
                if close > 0:
                    reward += (1 / (distance + 1)) * (1 - angle / (self.h_fov / 2))
                # 角度偏移
                else:
                    reward += -(1 / (distance + 1) + 1) * (1 - angle / (self.h_fov / 2))
            else:
                # 距离靠近
        ...
        # 设置奖励:障碍物
        # 距离 TODO:Urep = 0.5 * η * (1/d)
        # print("coll_reward: ", -0.5 * (1 / distance2))
