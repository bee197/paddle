...
class RobotEnv(gym.Env):
    ...
    def reset(self):
        ...
        # 获得水平视野
        self.h_fov = self.__get_view_range()
        # 初始化计数器
        self.step_num = 0
        # 设置一步的时间,默认是1/240
        ...
        # 初始化障碍物2
        self.collision = self.__create_coll(seed2, obj="Bottle.obj", is_current_folder=True, scale=[0.005, 0.005, 0.005])
        # 初始化机器人
        ...
        self.distance_prev, distance_ori, distance_coll, robot_pos, coll_pos, robot_ori, coll_ori = self.__get_distance(self.collision)
    ...
    def step(self, action):
        ...
        # 获得距离
        distance, distance_ori, distance_coll, robot_pos, coll_pos, robot_ori, coll_ori = self.__get_distance(self.soccer)
        distance2, distance_ori2, distance_coll2, robot_pos2, coll_pos2, robot_ori2, coll_ori2 = self.__get_distance(self.collision)
        # 获得angle
        robot_angle = math.atan2(robot_ori[0], robot_ori[1]) / 3.14
        coll_angle = math.atan2(coll_pos[0], coll_pos[1]) / 3.14
        ...
        angle2 = np.abs(math.atan2(coll_pos2[0], coll_pos2[1]) - math.atan2(robot_ori2[0], robot_ori2[1]))
        # print("angle : ", angle)
        ...
        reward = 0
        # 障碍物
        video_in = angle2 < (self.h_fov / 2)
        if video_in:
            close = self.angle_prev - angle2
            # 角度较大时考虑角度奖励
            if angle2 > 0.24:
                # 角度靠中心了
                if close > 0:
                    reward -= (1 / (distance2 + 1)) * (1 - angle2 / (self.h_fov / 2))
                # 远离中心
                else:
                    reward += -(1 / (distance2 + 1) + 1) * (1 - angle2 / (self.h_fov / 2))
            # 角度较小时考虑距离奖励
            else:
                # 前进了
                if self.distance_prev - distance2 > 0:
                    reward -= 1 / (distance2 + 1)
                # 没前进
                else:
                    reward += -(1 / (distance2 + 1) + 1)
        # print(f"rewawrd:{reward}")
        # 足球的奖励
        ...
        self.distance_prev = distance2
        self.angle_prev = angle2
