...
class RobotEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if k * self.coord2[0] <= self.coord1[1]:
            ...
            bias = [-1, 1]
        # 局部目标点
        distance3, distance_ori3, distance_coll3, robot_pos3, coll_pos3, robot_ori3, coll_ori3 = self.__get_distance(
            self.collision, bias=bias)
        # 获得angle TODO:
        # 障碍物与机器人向量
        ...
        coll_robot3 = np.array(coll_pos3) - np.array(robot_pos3)
        robot_angle3 = math.atan2(robot_ori3[0], robot_ori3[1])
        coll_angle3 = math.atan2(coll_robot3[0], coll_robot3[1])

        # TODO:
        angle = np.abs(coll_angle - robot_angle)
        angle2 = np.abs(coll_angle2 - robot_angle2)
        angle3 = np.abs(coll_angle3 - robot_angle3)
        ...
        if video_in2:
            ...
            if angle2 > 0.24:
                # 角度靠近
                if close > 0:
                    reward_coll -= (1 / (distance2 + 1)) * (1 - angle2 / (self.h_fov / 2))
                # 角度偏移
                else:
                    reward_coll -= -(1 / (distance2 + 1) + 1) * (1 - angle2 / (self.h_fov / 2))
            else:
                # 距离靠近
                if self.distance_prev2 - distance2 > 0:
                    reward_coll -= 1 / (distance2 + 1)
                # 距离偏移
                else:
                    reward_coll -= -(1 / (distance2 + 1) + 1)
        else:
            reward_coll += 1
        reward = reward_goal + reward_coll
        info = False
